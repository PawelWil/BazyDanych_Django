...
-- żeby ją odpalić muszę tą linię 'USE AdventureWorks' uruchomić, poprzez jej zaznaczenie +
-- ctrl + ent
USE AdventureWorks
...
ORDER BY Color, Name -- sortowanie najpierw po kolumnie Color, a potem po Name - ale to mówi nam, że wszystko
                    -- się zaczyna od Color,a potem jak coś jest w tej samej kategprii Color do posortowania
                    -- od strony Name jest sortowane
...
SELECT * FROM Production.Product WHERE Color = 'Black' -- tu wyświetlamy wszystkie kolumny +
...
-- 1) AND - ma wyższe znaczenie niż OR, czyli jak będzie Or i And, to zawsze pierwszy będzie rozpatrywany argument
    -- który jest po And, a dopiero potem ten po Or.
-- 2) OR


-- W poniższym przypadku jak widzimy Color 'Black' jest połączony z 'ProductSubCatID' za pomocą AND,więc dostaniemy
-- wszystkie kolory 'black' z ProductSubcategoryID = 1,
-- zaś Color 'Red' nie jest połączony z ProdSubCatID=1, bo po sobie ma OR, więc dostaniemy kolor 'red'
-- ale dla wszystkich ProdSubCatID = 1,12,3,4,5,6 itd.
...
SELECT Name, Color FROM Production.Product
WHERE Color = 'Black' AND Color = 'Red' -- tu jest wykluczenie polegające na tym, że chcemy kolumnę Name i Color,
...
SELECT * FROM Production.Product
WHERE ProductSubcategoryID = 2 AND NOT Color = 'Red' -- tu mamy wyniki, gdzie mamy PrudSubCatID=2 i NIE kolory
-- czerwone
...
-- taki sam wynik, ale poprzez krótszą wersję, za pomocą IN i w nawiasie okrągłym podajemy argumenty do filtrowania
SELECT * FROM Production.Product
...
-- nawiasy - a teraz dzięki nawiasom można zdecydować kolejnośc wykonywania operacji. Wiadomo, że w nawiasie się
-- rzeczy wykonają razem. Czyli poniżej dwa podwójne warunki się wykonają
...
SELECT Color, ProductSubcategoryID, * FROM Production.Product
WHERE (Color IN ('Red', 'Black')) AND ProductSubcategoryID = 1 -- kolor red i black dla ProdSubCat=1
...
SELECT * FROM Sales.SalesOrderHeader
WHERE OrderDate BETWEEN '20040501' AND '20040507' -- tu filtrujemy po zakresie daty BETWEEN x AND y
-- taki sam wynik jak powyżej, dzięki użyciu porównań, ale powyższa składnia lepsza/preferowana
SELECT * FROM Sales.SalesOrderHeader
...
-- * Generalnie SQL sobie poradził zarówno z datą napisaną jako: '20040501' oraz jako '2004-05-01'.
...
FROM Person.Contact -- tu wyświetlamy sobie kolumnę FirstName, LastName + teraz używamy konkatenacji żeby połączyć
-- te 2 kolumny w jedną, która będzie od razu wyświetlać imię i nazwisko + zmieniamy jej nazwę za pomocą Aliasu (AS)
-- na FullName
